...
class RootFindingApp:
    def __init__(self, root):
        ...
        self.entries_A_comp = [];
        self.entries_b_comp = [];
        self.entries_x0_comp = []
        self.entry_tol_comp = None;
        self.entry_max_iter_comp = None;
        self.entry_w_comp = None
    ...
    def open_lu_window(self):
        ...
        btn_generate_matrix = ttk.Button(size_config_frame, text="Generar Matriz",
                                         command=self._generate_lu_matrix_entries)
        ...
        try:
            ...
            for i in range(self.lu_matrix_size):
                ...
                for j in range(self.lu_matrix_size):
                    ...
                    if not val:
                        self.results_lu_text.insert(tk.END,
                                                    f"Error: El valor A[{i + 1}][{j + 1}] no puede estar vacío.\n")
                        self.results_lu_text.config(state=tk.DISABLED)
                        return
            ...
            self.results_lu_text.insert(tk.END,
                                        "Error: Todos los valores de la matriz A y el vector b deben ser números válidos.\n")
        except IndexError:  # Puede ocurrir si entries_A o entries_b no se llenaron correctamente
            self.results_lu_text.insert(tk.END,
                                        "Error: Discrepancia en el tamaño de la matriz. Intente regenerar la matriz.\n")
    ...
    def open_newton_classic_window(self):
        ...
        self.newton_custom_functions_frame = ttk.LabelFrame(config_params_frame,
                                                            text="Definición de Funciones (Sistema Personalizado)")
        ...
        ttk.Label(jac_entry_frame, text="Jacobiano J(x,y):").grid(row=0, column=0, columnspan=4, padx=5, pady=5,
                                                                  sticky='w')
        ...
        self.entry_x0_newton = ttk.Entry(init_frame, width=8);
        self.entry_x0_newton.pack(side=tk.LEFT, padx=5);
        self.entry_x0_newton.insert(0, "1.0")
        self.entry_y0_newton = ttk.Entry(init_frame, width=8);
        self.entry_y0_newton.pack(side=tk.LEFT, padx=5);
        self.entry_y0_newton.insert(0, "1.0")
        ...
        self.entry_tol_newton = ttk.Entry(other_params_frame, width=10);
        self.entry_tol_newton.grid(row=0, column=1, padx=5, pady=2);
        self.entry_tol_newton.insert(0, "1e-6")
        ttk.Label(other_params_frame, text="Max. Iteraciones:").grid(row=0, column=2, padx=5, pady=2, sticky="w")
        self.entry_max_iter_newton = ttk.Entry(other_params_frame, width=10);
        self.entry_max_iter_newton.grid(row=0, column=3, padx=5, pady=2);
        self.entry_max_iter_newton.insert(0, "100")
        ttk.Label(other_params_frame, text="Factor Relajación (w):").grid(row=1, column=0, padx=5, pady=2, sticky="w")
        self.entry_w_newton = ttk.Entry(other_params_frame, width=10);
        self.entry_w_newton.grid(row=1, column=1, padx=5, pady=2);
        self.entry_w_newton.insert(0, "1.0")
        ...
        self.results_newton_text = tk.Text(results_frame, height=35, width=100,
                                           wrap=tk.WORD)  # Ajustar tamaño según necesidad
        ...
        self.summary_newton_text = tk.Text(summary_frame, height=15, width=100, wrap=tk.WORD,
                                           font=("Arial", 10))  # Ajustar tamaño y fuente
    ...
    def _set_custom_system_values_from_example(self):
        ...
        self.text_f1_eq.delete("1.0", tk.END);
        self.text_f1_eq.insert("1.0", default_f1)
        self.text_f2_eq.delete("1.0", tk.END);
        self.text_f2_eq.insert("1.0", default_f2)
        self.text_j11_eq.delete("1.0", tk.END);
        self.text_j11_eq.insert("1.0", default_j11)
        self.text_j12_eq.delete("1.0", tk.END);
        self.text_j12_eq.insert("1.0", default_j12)
        self.text_j21_eq.delete("1.0", tk.END);
        self.text_j21_eq.insert("1.0", default_j21)
        self.text_j22_eq.delete("1.0", tk.END);
        self.text_j22_eq.insert("1.0", default_j22)
    ...
    def solve_newton_classic_system(self):
        ...
        try:
                ...
                for item in hist:
                    ...
                    if x_k_val is not None:
                        detailed_content += "Vector x_k (punto de inicio de esta iteración):\n" + np.array2string(
                            x_k_val, precision=10, suppress_small=True, separator=', ') + "\n"
                    else:
                        detailed_content += "Vector x_k: N/A\n"

                    if fx_k_val is not None:
                        detailed_content += "Valor de F(x_k):\n" + np.array2string(fx_k_val, precision=10,
                                                                                   suppress_small=True,
                                                                                   separator=', ') + "\n"
                    else:
                    ...
                    if jx_k_val is not None:
                        detailed_content += "Jacobiano J(x_k):\n" + np.array2string(jx_k_val, precision=5,
                                                                                    suppress_small=True,
                    ...
                    if delta_k_val is not None:
                        detailed_content += "Paso delta_k:\n" + np.array2string(delta_k_val, precision=10,
                                                                                suppress_small=True,
                                                                                separator=', ') + "\n"
                    else:
                    ...
                    if x_k_val is not None and delta_k_val is not None:
                        ...
                        detailed_content += "Vector x_{k+1} (resultado de esta iteración):\n" + np.array2string(
                            x_k_siguiente, precision=10, suppress_small=True, separator=', ') + "\n"
                    else:
            ...
            if hasattr(self,
                       'results_newton_text') and self.results_newton_text and self.results_newton_text.winfo_exists():
                self.results_newton_text.config(state=tk.DISABLED)
            if hasattr(self,
                       'summary_newton_text') and self.summary_newton_text and self.summary_newton_text.winfo_exists():
                self.summary_newton_text.config(state=tk.DISABLED)
    ...
    def open_sor_window(self):
        ...
        self.sor_window = tk.Toplevel(self.root);
        self.sor_window.title("Gauss-Seidel / SOR")
        ...
        # Frame tamaño (arriba)
        size_frame = ttk.Frame(self.sor_window);
        size_frame.pack(pady=10)
        ttk.Label(size_frame, text="Tamaño (N x N):").pack(side=tk.LEFT, padx=5)
        self.sor_matrix_size_entry = ttk.Entry(size_frame, width=5);
        self.sor_matrix_size_entry.pack(side=tk.LEFT, padx=5);
        self.sor_matrix_size_entry.insert(0, "3")
        ttk.Button(size_frame, text="Generar Matriz", command=self._generate_sor_matrix_entries).pack(side=tk.LEFT,
                                                                                                      padx=5)
        ...
        params_frame = ttk.Frame(self.sor_window);
        params_frame.pack(pady=10)
        ttk.Label(params_frame, text="Factor Relajación (w):").grid(row=0, column=0, padx=5, pady=2, sticky="w")
        self.entry_w_sor = ttk.Entry(params_frame, width=10);
        self.entry_w_sor.grid(row=0, column=1, padx=5, pady=2);
        self.entry_w_sor.insert(0, "1.0")
        ttk.Label(params_frame, text="Tolerancia (tol):").grid(row=1, column=0, padx=5, pady=2, sticky="w")
        self.entry_tol_sor = ttk.Entry(params_frame, width=10);
        self.entry_tol_sor.grid(row=1, column=1, padx=5, pady=2);
        self.entry_tol_sor.insert(0, "1e-6")
        ttk.Label(params_frame, text="Max. Iteraciones:").grid(row=2, column=0, padx=5, pady=2, sticky="w")
        self.entry_max_iter_sor = ttk.Entry(params_frame, width=10);
        self.entry_max_iter_sor.grid(row=2, column=1, padx=5, pady=2);
        self.entry_max_iter_sor.insert(0, "100")
        ...
        self.results_sor_text = tk.Text(self.sor_window, height=15, width=85);
        self.results_sor_text.pack(pady=5, padx=5, fill=tk.BOTH, expand=True);
        self.results_sor_text.config(state=tk.DISABLED)

        self._generate_sor_matrix_entries()

    def _generate_sor_matrix_entries(self):
        try:
            n = int(self.sor_matrix_size_entry.get()); assert 2 <= n <= 10; self.sor_matrix_size = n
        except:
            self._show_error_in_text(self.results_sor_text, "Tamaño de matriz inválido (2-10)."); return
    ...
    def solve_sor_system(self):
        ...
        try:
            A_list = [[float(self.entries_A_sor[i][j].get()) for j in range(self.sor_matrix_size)] for i in
                      range(self.sor_matrix_size)]
            ...
            self._show_error_in_text(self.results_sor_text,
                                     "Error: Valores numéricos inválidos para matrices, x0 o parámetros.")
        except Exception as e:
            self._show_error_in_text(self.results_sor_text, f"Error inesperado (SOR): {e}")

    def open_jacobi_window(self):
        if self.jacobi_window and self.jacobi_window.winfo_exists(): self.jacobi_window.lift(); return
        self.jacobi_window = tk.Toplevel(self.root);
        self.jacobi_window.title("Método de Jacobi Clásico")
        self.jacobi_window.geometry("750x750")
        self.jacobi_window.protocol("WM_DELETE_WINDOW",
                                    lambda: self._close_window_generic(self.jacobi_window, 'jacobi_window'))

        # Frame tamaño (arriba)
        size_frame = ttk.Frame(self.jacobi_window);
        size_frame.pack(pady=10)
        ttk.Label(size_frame, text="Tamaño (N x N):").pack(side=tk.LEFT, padx=5)
        self.jacobi_matrix_size_entry = ttk.Entry(size_frame, width=5);
        self.jacobi_matrix_size_entry.pack(side=tk.LEFT, padx=5);
        self.jacobi_matrix_size_entry.insert(0, "3")
        ttk.Button(size_frame, text="Generar Matriz", command=self._generate_jacobi_matrix_entries).pack(side=tk.LEFT,
                                                                                                         padx=5)
        ...
        # Frame para parámetros (debajo del área de entrada)
        params_frame = ttk.Frame(self.jacobi_window);
        params_frame.pack(pady=10)
        ttk.Label(params_frame, text="Tolerancia (tol):").grid(row=0, column=0, padx=5, pady=2, sticky="w")
        self.entry_tol_jacobi = ttk.Entry(params_frame, width=10);
        self.entry_tol_jacobi.grid(row=0, column=1, padx=5, pady=2)
        ...
        self.entry_max_iter_jacobi = ttk.Entry(params_frame, width=10);
        self.entry_max_iter_jacobi.grid(row=1, column=1, padx=5, pady=2)
        ...
        self.results_jacobi_text = tk.Text(self.jacobi_window, height=15, width=85);
        self.results_jacobi_text.pack(pady=5, padx=5, fill=tk.BOTH, expand=True);
        self.results_jacobi_text.config(state=tk.DISABLED)

        self._generate_jacobi_matrix_entries()  # Generar campos iniciales (A, b, x0)

    def _generate_jacobi_matrix_entries(self):
        try:
            n = int(self.jacobi_matrix_size_entry.get()); assert 2 <= n <= 10; self.jacobi_matrix_size = n
        except:
            self._show_error_in_text(self.results_jacobi_text, "Tamaño de matriz inválido (2-10)."); return
    ...
    def solve_jacobi_system(self):
        ...
        try:
            A_list = [[float(self.entries_A_jacobi[i][j].get()) for j in range(self.jacobi_matrix_size)] for i in
                      range(self.jacobi_matrix_size)]
            ...
            self._show_error_in_text(self.results_jacobi_text,
                                     "Error: Valores numéricos inválidos para matrices, x0 o parámetros.")
        except Exception as e:
            self._show_error_in_text(self.results_jacobi_text, f"Error inesperado (Jacobi): {e}")
    ...
    def open_compare_linear_window(self):
        ...
        self.compare_window.title("Comparar Métodos Lineales")
        self.compare_window.geometry("900x850")  # Más alto aún para el plot
        self.compare_window.protocol("WM_DELETE_WINDOW",
                                     lambda: self._close_window_generic(self.compare_window, 'compare_window'))

        # Frame tamaño
        size_frame = ttk.Frame(self.compare_window);
        size_frame.pack(pady=10)
        ttk.Label(size_frame, text="Tamaño (N x N):").pack(side=tk.LEFT, padx=5)
        self.compare_matrix_size_entry = ttk.Entry(size_frame, width=5);
        self.compare_matrix_size_entry.pack(side=tk.LEFT, padx=5);
        self.compare_matrix_size_entry.insert(0, "3")
        ttk.Button(size_frame, text="Generar Matriz", command=self._generate_compare_matrix_entries).pack(side=tk.LEFT,
                                                                                                          padx=5)

        # Frame contenedor A, b, x0
        self.compare_input_area_frame = ttk.Frame(self.compare_window);
        self.compare_input_area_frame.pack(pady=10)

        # Frame parámetros (tol, max_iter, w)
        params_frame = ttk.Frame(self.compare_window);
        params_frame.pack(pady=5)
        ttk.Label(params_frame, text="Tolerancia (tol):").grid(row=0, column=0, padx=5, pady=2, sticky="w")
        self.entry_tol_comp = ttk.Entry(params_frame, width=10);
        self.entry_tol_comp.grid(row=0, column=1, padx=5, pady=2);
        self.entry_tol_comp.insert(0, "1e-6")
        ttk.Label(params_frame, text="Max. Iteraciones:").grid(row=0, column=2, padx=5, pady=2, sticky="w")
        self.entry_max_iter_comp = ttk.Entry(params_frame, width=10);
        self.entry_max_iter_comp.grid(row=0, column=3, padx=5, pady=2);
        self.entry_max_iter_comp.insert(0, "100")
        ttk.Label(params_frame, text="Factor Relajación (w para SOR):").grid(row=1, column=0, padx=5, pady=2,
                                                                             sticky="w")
        self.entry_w_comp = ttk.Entry(params_frame, width=10);
        self.entry_w_comp.grid(row=1, column=1, padx=5, pady=2);
        self.entry_w_comp.insert(0, "1.0")
        ...
        frame_lu = ttk.Frame(self.compare_notebook, padding=5);
        self.compare_notebook.add(frame_lu, text='Factorización LU')
        self.compare_results_text_lu = tk.Text(frame_lu, height=20, width=100, wrap=tk.WORD);
        self.compare_results_text_lu.pack(fill=tk.BOTH, expand=True);
        self.compare_results_text_lu.config(state=tk.DISABLED)
        frame_jacobi = ttk.Frame(self.compare_notebook, padding=5);
        self.compare_notebook.add(frame_jacobi, text='Jacobi Clásico')
        self.compare_results_text_jacobi = tk.Text(frame_jacobi, height=20, width=100, wrap=tk.WORD);
        self.compare_results_text_jacobi.pack(fill=tk.BOTH, expand=True);
        self.compare_results_text_jacobi.config(state=tk.DISABLED)
        frame_sor = ttk.Frame(self.compare_notebook, padding=5);
        self.compare_notebook.add(frame_sor, text='Gauss-Seidel / SOR')
        self.compare_results_text_sor = tk.Text(frame_sor, height=20, width=100, wrap=tk.WORD);
        self.compare_results_text_sor.pack(fill=tk.BOTH, expand=True);
        self.compare_results_text_sor.config(state=tk.DISABLED)
    ...
    def _generate_compare_matrix_entries(self):
        try:
            n = int(self.compare_matrix_size_entry.get()); assert 2 <= n <= 10; self.compare_matrix_size = n
        except:
            self._show_error_in_text(self.compare_results_text_lu,
                                     "Tamaño inválido (2-10). Error mostrado en pestaña LU."); return
        ...
        self.entries_b_comp = [];
        self.entries_x0_comp = []
        matrix_a_frame = ttk.LabelFrame(self.compare_input_area_frame, text="Matriz A");
        matrix_a_frame.pack(side=tk.LEFT, padx=10, pady=5, anchor='n')
        vector_b_frame = ttk.LabelFrame(self.compare_input_area_frame, text="Vector b");
        vector_b_frame.pack(side=tk.LEFT, padx=10, pady=5, anchor='n')
        vector_x0_frame = ttk.LabelFrame(self.compare_input_area_frame, text="Estimación Inicial x0 (Jacobi/SOR)");
        vector_x0_frame.pack(side=tk.LEFT, padx=10, pady=5, anchor='n')
        for i in range(self.compare_matrix_size):
            row_entries_a = [];
            self.entries_A_comp.append(row_entries_a)
            for j in range(self.compare_matrix_size): entry_a = ttk.Entry(matrix_a_frame, width=5); entry_a.grid(row=i,
                                                                                                                 column=j,
                                                                                                                 padx=2,
                                                                                                                 pady=2); row_entries_a.append(
                entry_a)
            entry_b = ttk.Entry(vector_b_frame, width=5);
            entry_b.grid(row=i, column=0, padx=2, pady=2);
            self.entries_b_comp.append(entry_b)
            entry_x0 = ttk.Entry(vector_x0_frame, width=8);
            entry_x0.grid(row=i, column=0, padx=2, pady=2);
            entry_x0.insert(0, "0.0");
            self.entries_x0_comp.append(entry_x0)
    ...
    def solve_compare_linear(self):
        ...
        try:
            A_list = [[float(self.entries_A_comp[i][j].get()) for j in range(self.compare_matrix_size)] for i in
                      range(self.compare_matrix_size)]
            b_list = [float(self.entries_b_comp[i].get()) for i in range(self.compare_matrix_size)]
            x0_list = [];
            for i in range(self.compare_matrix_size): val_str = self.entries_x0_comp[i].get(); x0_list.append(
                float(val_str) if val_str else 0.0)
            tol = float(self.entry_tol_comp.get());
            max_iter = int(self.entry_max_iter_comp.get());
            w_sor = float(self.entry_w_comp.get())
            A_np = np.array(A_list, dtype=float);
            b_np = np.array(b_list, dtype=float);
            x0_np = np.array(x0_list, dtype=float)
            ...
            try:
                ...
                self._show_error_in_text(self.compare_results_text_lu, f"Error ejecutando LU: {e_lu}")
            ...
            try:
                ...
                self._show_error_in_text(self.compare_results_text_jacobi, f"Error ejecutando Jacobi: {e_jacobi}")
            ...
            try:
                ...
                self._display_iterative_results(self.compare_results_text_sor, resultado_sor, self.compare_matrix_size,
                                                append_summary=False)
            except Exception as e_sor:
                self._show_error_in_text(self.compare_results_text_sor, f"Error ejecutando SOR: {e_sor}")

            # --- Actualizar Pestaña de Plot ---
            self._update_plot_tab(self.compare_matrix_size, A_np, b_np, solution_x)

        except ValueError:
            self._show_error_in_text(self.compare_results_text_lu, "Error: Valores numéricos inválidos...")
        except Exception as e:
            self._show_error_in_text(self.compare_results_text_lu, f"Error inesperado en Comparación: {e}")
    ...
    def _update_plot_tab(self, N, A_np, b_np, solution_x):
        ...
        if solution_x is None:
            ttk.Label(self.compare_plot_frame,
                      text="No se pudo obtener la solución (ej. matriz singular). No se puede graficar.").pack(padx=10,
                                                                                                               pady=10)
            return
        ...
        if N == 2:
            ...
            ttk.Label(self.compare_plot_frame, text=f"Visualización gráfica no disponible para sistemas {N}x{N}.").pack(
                padx=10, pady=10)
            ...
            for i in range(3):
                ...
                if not np.isclose(c_coeff, 0):  # Plano normal: z = (d - ax - by) / c
                    ...
                    ax.plot_surface(x_surf, y_surf, z_surf, color=colors[i], alpha=alphas,
                                    label=f'Ec {i + 1}', rstride=100,
                                    cstride=100)  # rstride/cstride bajos para visualización
        ...
        if isinstance(resultado, str):  # Error de validación inicial del backend
            ...
            if not historial:
                ...
                for item in historial:
                    ...
                    x_k_str_oneline = ' '.join(
                        np.array2string(x_k_vec, precision=10, suppress_small=True, separator=', ').replace('\n',
                                                                                                            '').split())
